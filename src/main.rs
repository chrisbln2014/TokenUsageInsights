...
#[tokio::main]
async fn main() {
    // cli::run 會把 --export-snapshot 視為未知指令，必須先攔截
    if let Some(path) = snapshot::export_snapshot_path_from_args() {
        std::process::exit(export_snapshot(&path).await);
    }
    ...
    // Cloud Run 唯讀模式沒有本機安裝與 SQLite，不可進入下方的更新救援、資料庫初始化與自動更新流程
    if snapshot::snapshot_mode_enabled() {
        serve_snapshot_dashboard().await;
        return;
    }
...
async fn export_snapshot(path: &std::path::Path) -> i32 {
    let mut conn = match initialize_database_schema().and_then(|()| db::get_db_conn()) {
        Ok(conn) => conn,
        Err(error) => {
            eprintln!("❌ Snapshot 匯出失敗，無法初始化 SQLite 資料庫: {error}");
            return 1;
        }
    };
    if let Err(error) = db::sync_usage_logs(&mut conn) {
        eprintln!("❌ 匯出前同步日誌檔到 SQLite 失敗，將以現有資料匯出: {error}");
    }
    drop(conn);
    match snapshot::write_snapshot_file(path).await {
        Ok(()) => {
            println!("✅ Snapshot 已匯出: {path:?}");
            0
        }
        Err(error) => {
            eprintln!("❌ Snapshot 匯出失敗: {error}");
            1
        }
    }
}

async fn serve_snapshot_dashboard() {
    println!("☁️ 已啟用 Cloud Run snapshot 模式，將從 Google Drive 或 snapshot 檔案讀取資料。");
    let static_dir = get_static_dir();
    println!("📂 正在服務靜態檔案，目錄來源: {:?}", static_dir);
    let app = build_snapshot_router(&static_dir).layer(build_cors_layer());
    ...
    println!(
        "🚀 Token 戰情室 (snapshot) is running on: {}",
        browser_url_for_bind_address(bind_address)
    );
...
fn build_snapshot_router(static_dir: &PathBuf) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/api/:assistant/usage/:date",
            get(snapshot::get_usage_details),
        )
        .route(
            "/api/:assistant/usage/:date/session-search",
            get(snapshot::unsupported_in_snapshot_mode),
        )
        .route(
            "/api/:assistant/usage/:date/export",
            get(snapshot::unsupported_in_snapshot_mode),
        )
        .route(
            "/api/:assistant/usage/:date/import",
            post(snapshot::unsupported_in_snapshot_mode),
        )
        .route(
            "/api/:assistant/imports",
            get(snapshot::unsupported_in_snapshot_mode),
        )
        .route(
            "/api/:assistant/imports/:batch_id",
            delete(snapshot::unsupported_in_snapshot_mode),
        )
        .route(
            "/api/:assistant/session/:session_id",
        ...
        .route(
            "/api/:assistant/months",
            get(snapshot::get_available_months),
        )
        .route(
            "/api/:assistant/monthly/:year_month",
...
mod tests {
    ...
    fn frontend_api_paths() -> Vec<String> {
        ...
        for (start, _) in app_js.match_indices("/api/") {
            let mut path = String::new();
            let mut chars = app_js[start..].chars().peekable();
    ...
    #[tokio::test]
    async fn snapshot_router_answers_every_frontend_api_path_with_json() {
        ...
        let expectations = [
            ...
            (Method::GET, "/api/x/usage/x", false),
            (Method::GET, "/api/x/monthly/x", false),
            (Method::GET, "/api/x/yearly/x", false),
            (Method::GET, "/api/x/setup-info", false),
            (Method::GET, "/api/x/pricing", false),
            (Method::GET, "/api/x/sync", false),
            (Method::GET, "/api/x/session/xx", false),
            (Method::GET, "/api/x/model-sessions", true),
            (Method::GET, "/api/x/usage/x/session-search", true),
            (Method::GET, "/api/x/usage/x/export", true),
            (Method::POST, "/api/x/usage/all/import", true),
            (Method::GET, "/api/x/imports", true),
            (Method::DELETE, "/api/x/imports/x", true),
        ];

        let mut frontend_paths = frontend_api_paths();
        frontend_paths.sort();
        let mut expected_paths: Vec<String> = expectations
            .iter()
            .map(|(_, path, _)| path.to_string())
            .collect();
        expected_paths.sort();
        assert_eq!(
            frontend_paths, expected_paths,
            "app.js 的 API 路徑與 snapshot 路由預期表不一致，請為新端點決定 snapshot 模式行為"
        );

        let router = build_snapshot_router(&PathBuf::from("static"));
        for (method, path, unsupported) in expectations {
            let response = router
                .clone()
                .oneshot(
                    Request::builder()
            ...
            assert!(
                is_json,
                "{method} {path} 未回 JSON（路由缺漏或方法錯誤）: {status}"
            );
            if unsupported {
